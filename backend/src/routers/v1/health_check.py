...
@router_health_check.get("/health", include_in_schema=False, status_code=status.HTTP_200_OK)
    ...
    try:
        return JSONResponse(
            content={
                ...
                "api_urls": {
                    "api_docs": f"/api/v{env.APP_VERSION}/docs",
                    "alternative_api_docs": f"/api/v{env.APP_VERSION}/redoc",
                }
            },
            status_code=status.HTTP_200_OK
        )
